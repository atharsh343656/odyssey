...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (scheduleForm) {
        ...
        scheduleForm.addEventListener('submit', (e) => {
            ...
            const editId = scheduleEditId ? scheduleEditId.value : '';
            ...
            if (editId) {
                const index = schedule.findIndex(s => s.id === editId);
                if (index !== -1) {
                    schedule[index] = { ...schedule[index], time, event, category, venue };
                }
                if (scheduleEditId) scheduleEditId.value = '';
                if (scheduleSubmitBtn) scheduleSubmitBtn.innerHTML = 'Save Schedule <i data-lucide="calendar"></i>';
            ...
            loadAdminSchedule();
            showSuccess(scheduleSubmitBtn || scheduleForm.querySelector('button'), editId ? 'Updated!' : 'Saved!');
            scheduleForm.reset();
            lucide.createIcons();
        });
        ...
        if (resetScheduleBtn) {
            resetScheduleBtn.addEventListener('click', () => {
                if (confirm('Are you sure you want to clear the entire schedule? This cannot be undone.')) {
                    localStorage.removeItem('odyssey2k26_schedule');
                    loadAdminSchedule();
                }
            });
        }
            ...
            tr.innerHTML = `
                ...
                <td>
                    <div style="display: flex; gap: 10px;">
                        <button class="edit-btn edit-schedule-btn" data-id="${item.id}" title="Edit" style="background: none; border: none; color: var(--secondary); cursor: pointer;">
        ...
        document.querySelectorAll('.edit-schedule-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                if (item) {
                    ...
                    document.getElementById('schedule-event').value = item.event;
                    document.getElementById('schedule-category').value = item.category;
                    document.getElementById('schedule-venue').value = item.venue;
                    if (scheduleEditId) scheduleEditId.value = item.id;
                    if (scheduleSubmitBtn) scheduleSubmitBtn.innerHTML = 'Update Schedule <i data-lucide="save"></i>';
                    if (scheduleCancelBtn) scheduleCancelBtn.style.display = 'block';
                    lucide.createIcons();
                    window.scrollTo({ top: scheduleForm.offsetTop - 100, behavior: 'smooth' });
                }
            });
        });
    }
